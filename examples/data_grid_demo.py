...
def main() -> int:
    ...
    title = Label(
        "SwirUI DataGrid — retained row virtualization",
        bounds=Rect(32.0, 24.0, 760.0, 38.0),
        font_size=25.0,
        color=Color.from_hex("#62E5FF"),
    )
    status = Label(
        "Click a row, then use ↑ ↓ Home End PageUp PageDown.",
        bounds=Rect(32.0, 68.0, 760.0, 28.0),
        font_size=15.0,
        color=Color.from_hex("#DDF5FF"),
    )
    columns = (
        DataGridColumn("id", "ID", width=82.0),
        DataGridColumn("task", "Task", width=360.0),
