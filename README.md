...
**54% — 0.4 Alpha Core Widgets underway; retained scrolling, disclosure, split-container, status, tooltip and surface controls are implemented and verified.**
...
`python desktop gui` • `python gpu ui` • `native python ui framework` • `python retained widgets` • `python scrollview widget` • `python accordion widget` • `python splitview widget` • `wgpu python renderer` • `rust pyo3 gui` • `win32 python gui` • `reactive desktop ui` • `high refresh rate ui` • `hidpi desktop ui` • `gpu text rendering` • `frosted glass ui` • `acrylic desktop ui` • `python glass card ui` • `custom wgsl effects` • `multi monitor python ui`
