...
pub(crate) struct BackdropCompositor {
    sampler: wgpu::Sampler,
    bind_group_layout: wgpu::BindGroupLayout,
    pipeline: wgpu::RenderPipeline,
    params: wgpu::Buffer,
...
impl BackdropCompositor {
    pub(crate) fn new(
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("SwirUI backdrop blur linear sampler"),
            mag_filter: wgpu::FilterMode::Linear,
            min_filter: wgpu::FilterMode::Linear,
            ..Default::default()
        });
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("SwirUI backdrop compositor bind group layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::FRAGMENT,
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("SwirUI backdrop compositor pipeline layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("SwirUI rounded backdrop compositor shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("backdrop_blur.wgsl").into()),
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("SwirUI persistent backdrop compositor pipeline"),
            layout: Some(&pipeline_layout),
            vertex: wgpu::VertexState {
                module: &shader,
                entry_point: Some("vs_main"),
        ...
        let params = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("SwirUI backdrop compositor parameters"),
            contents: &floats_to_bytes(&[0.0; 12]),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        let bind_group = create_backdrop_bind_group(
            device,
            &bind_group_layout,
            source_view,
            &sampler,
    ...
    pub(crate) fn rebind_source(
        ...
        self.bind_group = create_backdrop_bind_group(
            device,
            &self.bind_group_layout,
            source_view,
            &self.sampler,
    ...
    pub(crate) fn encode(
        ...
        let values = backdrop_params(viewport, origin, size, radii);
        queue.write_buffer(&self.params, 0, &floats_to_bytes(&values));
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("SwirUI rounded backdrop composite pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: target,
                depth_slice: None,
                resolve_target: None,
...
fn create_backdrop_bind_group(
    ...
    device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("SwirUI backdrop compositor bind group"),
        layout,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
...
mod tests {
    ...
    #[test]
    fn backdrop_uniform_layout_keeps_wgsl_alignment_padding() {
        let values = backdrop_params(
            (800, 600),
            (20.0, 30.0),
            (200.0, 120.0),
            [8.0, 10.0, 12.0, 14.0],
        );
        assert_eq!(values[0..6], [800.0, 600.0, 20.0, 30.0, 200.0, 120.0]);
        assert_eq!(values[6..8], [0.0, 0.0]);
        assert_eq!(values[8..12], [8.0, 10.0, 12.0, 14.0]);
    }
}
