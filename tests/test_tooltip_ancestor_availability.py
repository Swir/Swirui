...
def test_tooltip_refreshes_ancestor_observers_after_reparenting() -> None:
    first_parent = Component("first")
    second_parent = Component("second")
    button = Button("Inspect", bounds=Rect(0.0, 0.0, 120.0, 40.0))
    ...
    second_parent.add(button)
    second_parent.enabled = False
    assert tooltip.is_open is False

    second_parent.enabled = True
    button.emit("pointer_enter")
    assert tooltip.is_open is True
    first_parent.enabled = False
    assert tooltip.is_open is True
